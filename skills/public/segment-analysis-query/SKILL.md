...
2. 用默认口径补齐缺失信息。
3. 先判断问题应走 P&L/Segment 表还是 BS 表；需要字段名时读取对应 schema reference，需要指标中文名或别名时读取 `references/metric_names.md`。
4. 使用可用的 PostgreSQL 数据库查询工具查询对应事实表。
5. 用中文输出结论、关键依据和必要口径。
...
- 默认按月度展示。
- 默认按 `line_no` 展示 BS 科目顺序。
- 金额类结果标注单位“千元”，周转天数标注单位“天”。
- 原 Excel 的 check 校验列不进入 BS 问数表。
- 不要把合计项和明细项重复加总。
...
- 输出环境支持颜色或样式时，对关键结果做颜色提示：业务不利用红色，业务有利用绿色。
- 纯文本环境无法显示颜色时，用“有利”“不利”“改善”“恶化”等标签替代。
- 颜色判断要看业务含义，不要机械按正负号判断。
- 常见方向：收入、利润、现金、资产质量改善、周转效率提升通常越高越有利；费用、成本、负债、借款、周转天数上升通常越高越不利。
- 方向不明确时保持中性，不强行上色。
